...
from src.model import CustomSEResNeXt, CustomSEResNeXtMrk2
...
data_transforms_v2 = albumentations.Compose([
    albumentations.Transpose(p=0.5),
    albumentations.VerticalFlip(p=0.5),
    albumentations.HorizontalFlip(p=0.5),
...
class PANDADataset:
    ...
    def __getitem__(self, item):
        ...
        if self.transform is not None:
            ...
            augmented = self.transform_v2(image=images)
...
def run_one_fold(fold_id):
    ...
    train_dataset = PANDADataset(df=df_train, indices=train_idx, 
                                 transform=[data_transforms, data_transforms_v2])
    ...
    val_dataset = PANDADataset(df=df_train, indices=val_idx, 
                               transform=[data_transforms_test, data_transforms_test])
    ...
    model = CustomSEResNeXtMrk2(model_name='se_resnext50_32x4d', num_classes=5)
...
if __name__ == '__main__':
    ...
    LOGGER.info(f'{EXP_ID} : exp2 (LB 0.85) + apply 5d target cutout/shift rotate, patience=4')    
